...
function isItemFechamentoMensal(item) {
  const referenciaExternaId = String(item?.referenciaExternaId || "");
  return (
    referenciaExternaId.startsWith("fechamento:") ||
    referenciaExternaId.startsWith("maisquiosque:")
  );
}
  ...
  const syncMutation = useMutation({
    mutationFn: () =>
      integracaoApi.syncIntegracao(empresaSelecionada?.integracao, {
        ...
        referenceMonth:
          isMaisQuiosqueSelecionada && referenceMonth
            ? referenceMonth
            : undefined,
      }),
    onSuccess: () => {
      ...
      <div className="glass card-shadow rounded-2xl p-5 space-y-4">
        ...
        {isMaisQuiosqueSelecionada && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Input
              label="Mês de Referência da Sincronização"
              type="month"
              value={referenceMonth}
              onChange={(e) => setReferenceMonth(e.target.value)}
            />
            <p className="text-xs text-slate-400 md:col-span-2 md:self-end md:pb-2">
              Ao sincronizar, o sistema busca o fechamento desse mês na
              MaisQuiosque e cria pendências para você aprovar como gasto ou
              entrada.
            </p>
          </div>
        )}
